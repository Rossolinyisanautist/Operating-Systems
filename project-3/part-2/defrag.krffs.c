...
int main(int argc, char **argv)
{
	...
	puts("START DEFRAG");	
	struct krffs_node* curr = file_system.node;
	struct krffs_node* next = krffs_get_next_node(&file_system, curr);
	struct krffs_node* next2;
	
	do {
		next = krffs_get_next_node(&file_system, curr);
		if(next  == file_system.node)
		{
			puts("done.");
			break;
		}
		...
		switch(curr->type)
		{
				...
				{
					puts("free node!");
					puts("check next node");
					if(next->type == KRFFS_Free_Node)
					{
							puts("also free\nmerging...");
							// merge
							curr->size += next->size;
							// flush 
							krffs_sync_mapping(curr, sizeof(*curr));

							// update next.next->prev_size 
							next2 = krffs_get_next_node(&file_system, curr);
							next2->previous_node_size = curr->size;
							krffs_sync_mapping(next2, sizeof(*next2));

					}
					else
					{
							puts("next is reserved\nswapping...");
							uint64_t old_curr_size = curr->size;
							uint64_t prev_size = curr->previous_node_size;

							memmove(curr, next, next->size);
							//krffs_allocate_reserved_node(&file_system, next-size, next);

							curr->previous_node_size = prev_size;
							
							krffs_sync_mapping(curr, sizeof(*curr));

							next = krffs_get_next_node(&file_system, curr);
							next = krffs_initialize_free_node(next, old_curr_size, curr->size);
							

							next2 = krffs_get_next_node(&file_system, next);
							next2->previous_node_size = next->size;
							krffs_sync_mapping(next2, sizeof(*next2));
							
							puts("swapped. goto next node");
							curr = next;
					}
				...
				default:
				{
					puts("not free. goto next node");
					curr = next;
				}
		}
	} while(curr != file_system.node);
